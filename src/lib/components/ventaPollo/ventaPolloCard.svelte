<script lang="ts">
	...
	import type { VentaPollo, DropdownOption } from '$lib/interfaces';
	import { formatDate } from '$lib/utils';
	import { DropdownMenu } from '../ui';
	interface Props {
		nro: number;
		ventaPollo: VentaPollo;
		onEdit: (ventaPollo: VentaPollo) => void;
		onDelete: (id: string) => void;
	}

	let { nro, ventaPollo, onEdit, onDelete }: Props = $props();
	...
	const options: DropdownOption[] = [
		...
		},
		{
			...
			action: () => onDelete(ventaPollo.id), // id is string now
			divider: true,
			disabled: false
		}
	];
...
<tr class="hover:bg-light-one_d">
	...
	<td class="px-6 py-4 text-sm whitespace-nowrap text-light-two_d">
		{ventaPollo.Despacho.promotor.nombre} {ventaPollo.Despacho.promotor.apellido_paterno}
	</td>
	<td class="px-6 py-4 text-sm font-medium whitespace-nowrap text-light-two_d">
		{ventaPollo.Cliente.nombre} {ventaPollo.Cliente.apellido_paterno}
	</td>
	<td class="px-6 py-4 text-sm whitespace-nowrap text-light-two">
		{ventaPollo.Despacho.code_sku || "_"}
	</td>
	<!-- <td class="px-6 py-4 text-sm whitespace-nowrap text-light-two">
		{ventaPollo.zona}
	</td> -->
	<td class="px-6 py-4 text-sm whitespace-nowrap text-light-two">
		{ventaPollo.total_venta}
	</td>
	<td class="px-6 py-4 text-sm whitespace-nowrap text-light-two">
		{ventaPollo.saldo_pendiente}
	</td>
	<td class="px-6 py-4 text-sm whitespace-nowrap text-light-two">
		{formatDate(ventaPollo.fecha_venta)}
	</td>
